...
void doReweight_dk2nu(const char* inputFile, const char* outputFile, const char* optionsFile, const char* cxxdet, const char* cyydet, const char* czzdet){
  ...
  for(int ii=0;ii<Nnuhel;ii++){
    ...
    for(int jj=0;jj< Nuniverses; jj++){
      ...
      hthin_nua_datavol[ii][jj] = new TH1D(Form("hthin_nua_datavol_%s_%d", nuhel[ii], jj), Form("%s flux corrected based on thin target data on N+detector material, univ. #%i", nulabel[ii], jj), NbinsE, emin, emax);
      hthin_nua_datavol[ii][jj]->SetXTitle(xtitle);
      hthin_nua_datavol[ii][jj]->SetYTitle(ytitle);

      hthin_nua_othervol[ii][jj] = new TH1D(Form("hthin_nua_othervol_%s_%d", nuhel[ii], jj), Form("%s flux corrected based on thin target data on N+detector material, univ. #%i", nulabel[ii], jj), NbinsE, emin, emax);
      hthin_nua_othervol[ii][jj]->SetXTitle(xtitle);
      hthin_nua_othervol[ii][jj]->SetYTitle(ytitle);
  ...
  std::vector<double> vwgt_ttnua_datavol;
  std::vector<double> vwgt_ttnua_othervol;
  ...
  for(int ii=0;ii<nentries;ii++) {
    ...
    vwgt_ttnua.clear();
    vwgt_ttnua_datavol.clear();
    vwgt_ttnua_othervol.clear();
    // vwgt_ttnuAlFe.clear();
    ...
    vwgt_ttnua_datavol    = makerew->GetWeights("ThinTargetnucleonADataVol");
    vwgt_ttnua_othervol   = makerew->GetWeights("ThinTargetnucleonAOtherVol");
...
/*        *vwgt_ttmesinc_projectile_pip[jj]
      ...
      hthin_nua_datavol[nuidx][jj]->Fill(nuenergy, fluxWGT * vwgt_ttnua_datavol[jj]);
      hthin_nua_othervol[nuidx][jj]->Fill(nuenergy, fluxWGT * vwgt_ttnua_othervol[jj]);
  ...
  for(int ii=0;ii<Nnuhel;ii++){
    ...
    fOut->mkdir(Form("%s_thintarget/nua_datavol", nuhel[ii]));
    fOut->mkdir(Form("%s_thintarget/nua_othervol", nuhel[ii]));
  ...
  for(int ii=0;ii<Nnuhel;ii++){
    ...
    for(int jj=0;jj< Nuniverses; jj++){
      ...
      fOut->cd(Form("%s_thintarget/nua_datavol"    , nuhel[ii])); hthin_nua_datavol[ii][jj]->Write();
      fOut->cd(Form("%s_thintarget/nua_othervol"    , nuhel[ii])); hthin_nua_othervol[ii][jj]->Write();
